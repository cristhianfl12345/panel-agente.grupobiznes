...
export default function LeadRow({ lead, index, onCopy, columns = [], onTelefonoClick }) {
  ...
  return (
    <motion.td
      ...
      onClick={() => {
        ...
        onTelefonoClick?.(lead.numero_telefono)
      }}
    >
      <span
        className={`inline-block px-3 py-1 rounded-full font-medium text-sm shadow-sm transition-colors
...
return (
  <motion.tr
    ...
    style={{
      backgroundColor: hexToRGBA(lead.color_etiqueta, 0.15)
    }}

    whileHover={{
      backgroundColor: lead.color_etiqueta
        ? hexToRGBA(lead.color_etiqueta, 0.28)
        : isDark
          ? "rgba(40,44,59,0.9)"
